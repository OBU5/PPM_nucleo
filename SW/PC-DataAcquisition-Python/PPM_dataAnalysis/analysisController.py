...
def analyseDatafthroughUSB (port, baudrate, messageCountPerMeasurement, countOfMeasurements, fileName):
...
def analyseData(fh):
    ...
    enablePlot_measuredSignal = 1   #if enabled, the measured signal with ADC is stored as a picture
    enablePlot_comparator = 1       #if enabled, the measured signal with comparator is stored as a picture
        ...
        for line in fh:
            ...
            #found end of command    
                ...
                if procesingDataFromMeasurement == 1 and (commandArgs[2] == "extADC" or  commandArgs[2] == "intADC"):
                    ...
                    f_fft_zeroPadding1 = fa.computeFFTOfSignalWithZeroPadding(startSample = adc_startIndex, endSample = adc_stopIndex, signal = y, fs = fs, totalN = 4410000)
                    ...
                    f_fft_zeroPadding2 = fa.computeFFTOfSignalWithZeroPadding(startSample = adc_startIndex, endSample = adc_stopIndex, signal = y, fs = fs, totalN = 441000)
                    ...
                    if(enablePlot_measuredSignal):
                        ...
                        for k in range(len(y)):   
                            modifiedY.append((y[k] - 32768) * 4.096/65535)
                            modifiedX.append(x[k] / 44100)
    
                        fig = plt.figure()
                        plt.plot(modifiedX, modifiedY)   
                        plt.xlim(0, 2)
                        plt.yticks(np.arange(-5.0, 5.0, step=0.5))
                        plt.ylim(-4.096/2, 4.096/2)
         
                        plt.xlabel('Čas [s]') 
                        plt.ylabel('Napětí [V]')
                        plt.title('Časový průběh naměřeného signálu pomocí externího AD převodníku') 
                        plt.grid()
                        plt.show()
                        ...
                        #plt.title('Změřený signál') 

                        #plt.subplot(2, 5, 6)
                        #plt.xlim(0, 1800/44100)
                        #plt.plot(modifiedX, modifiedY)            
                        #plt.xlabel('Vzorky') 
                        #plt.ylabel('Amplituda')

                        #plt.subplot(2, 5, 7)
                        #plt.xlim(22000/44100, 23800/44100)
                        #plt.plot(modifiedX, modifiedY)            
                        #plt.xlabel('Vzorky') 
                        #plt.ylabel('Amplituda')

                        #plt.subplot(2, 5, 8)
                        #plt.xlim(44000/44100, 45800/44100)
                        #plt.plot(modifiedX, modifiedY)            
                        #plt.xlabel('Vzorky') 
                        #plt.ylabel('Amplituda')

                        #plt.subplot(2, 5, 9)
                        #plt.xlim(65100/44100, 66800/44100)
                        #plt.plot(modifiedX, modifiedY)            
                        #plt.xlabel('Vzorky') 
                        #plt.ylabel('Amplituda')

                        #plt.subplot(2, 5, 10)
                        #plt.xlim(86400/44100, 88200/44100)
                        #plt.plot(modifiedX, modifiedY)            
                        #plt.xlabel('Vzorky') 
                        #plt.ylabel('Amplituda')
                    ...
                    #print value
                    ###
                    f_comparator = fa.computeFrequencyWithComparator(signal= y, startSample= comparator_startIndex, endSample =comparator_endIndex, f_timer = 216000000)
                    ...
                    if(enablePlot_comparator):
                        ...
                        modifiedY = []
                        for k in range(len(y)):   
                            modifiedY.append((y[k]))
                            modifiedX.append(x[k] / 2000)
                        ...
                        plt.xlabel('Čas [s]') 
                        plt.ylabel('Hodnota časovače')
                        plt.title('Časový průběh naměřeného signálu pomocí komparátoru') 
                        plt.grid()
                        plt.show()
                        fig.savefig('generatedFiles/figures/' + graphTitle + '.png', dpi=fig.dpi)
                        plt.close(fig)
