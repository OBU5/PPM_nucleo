...
if(1)
    ...
    numberOfExaminedSegments = 10;
    samplesPerExaminedSegment = round(samplesPerPeriod)*8;
    ...
    minOfAlphaOptimizer = 0.5;
    maxOfAlphaOptimizer = 2;
    ...
    %----------------------------------------------------------------------
    amplitudePlotOn = false;
    amplitudeOptimizerOn = true;
    stepsOfAmplitudeOptimizer = 1000;
    minOfAmplitudeOptimizer = 0.5;
    maxOfAmplitudeOptimizer = 2;
    ...
    while(1)
        ...
        newOptimizedAlphaValues = zeros(1,numberOfExaminedSegments);
        newOptimizedOmegaValues = zeros(1,numberOfExaminedSegments);
        newOptimizedPhaseValues = zeros(1,numberOfExaminedSegments);
        newOptimizedAmplitudeValues = zeros(1,numberOfExaminedSegments);
        ...
        if(omegaOptimizerOn)
            ...
            for currentExaminedSegment = 1 : numberOfExaminedSegments
                
                % signalVector that is used in
                % sig=A*exp(alpha*(0:n-1)).*cos(w*(0:n-1)+ph);
                % it is used in order to divide examined signal into segments
                signalVector = floor(n*((currentExaminedSegment-1) / numberOfExaminedSegments))+1 :...
                    floor(n*((currentExaminedSegment-1) / numberOfExaminedSegments) + samplesPerExaminedSegment +1);
                ...
                for(k=1:length(omegaOptimizer))
                    ...
                    optimizedSignal=A*exp(alpha*(signalVector)).* cos((w*omegaOptimizer(k))*(signalVector)+phase);
                    
                    % create "evaluation" for each modification of alpha
                    evaluationOfOmegaOptimizer(k)=sum(abs(sig(signalVector(1):signalVector(end))-optimizedSignal(:)));
                end
                
                
                if(omegaPlotOn)
                    subplot(3,numberOfExaminedSegments,numberOfExaminedSegments + currentExaminedSegment)
                    plot(signalVector(1):signalVector(end),sig(signalVector(1):signalVector(end)),'r.-'...
                        ,signalVector(1):signalVector(end),optimizedSignal(:),'b.-');
                    title([num2str(currentExaminedSegment) '/' num2str(numberOfExaminedSegments) ' of sig']);

                    subplot(3,numberOfExaminedSegments,numberOfExaminedSegments+numberOfExaminedSegments + currentExaminedSegment)
                    plot(evaluationOfOmegaOptimizer)
                    title(['eval for ' num2str(currentExaminedSegment) '/' num2str(numberOfExaminedSegments) ' of sig']);
                end
                ...
                if(displayOn)
                    fprintf('\t\t\t%d. segment out of %d segments\n',currentExaminedSegment,numberOfExaminedSegments)
                    fprintf('\t\t\t\tomega optimizer = %f\n',omegaOptimizer(indexOmega))
                    fprintf('\n')
                end
                %Change omega to new optimized value
                newOptimizedOmegaValues(currentExaminedSegment)=omegaOptimizer(indexOmega) * w;
            end
            newValue = sum(newOptimizedOmegaValues(:))/length(newOptimizedOmegaValues);
            
            if(displayOn)
                fprintf('\t\tOptimized omega in %d. cycle -> changed from %f to %f\n', index, w, newValue);
                fprintf('\n');
            end
            w = newValue;
        end
        ...
        if(phaseOptimizerOn)
            ...
            for currentExaminedSegment = 1 : numberOfExaminedSegments
                ...
                for(k=1:length(phaseOptimizer))
                    ...
                    optimizedSignal=A*exp(alpha*(signalVector)).* cos(w*(signalVector)+(phase*phaseOptimizer(k)));
                    
                    % create "evaluation" for each modification of phase
                    evaluationOfPhaseOptimizer(k)=sum(abs(sig(signalVector(1):signalVector(end))-optimizedSignal(:)));
                end
                ...
                newOptimizedPhaseValues(currentExaminedSegment) = phaseOptimizer(indexPhase)*phase;
            end
            
            newValue = sum(newOptimizedPhaseValues(:))/length(newOptimizedPhaseValues);
            
            if(displayOn)
                fprintf('\t\tOptimized phase in %d. cycle -> changed from %f to %f\n', index, phase, newValue);
                fprintf('\n');
            end
            phase = newValue;
        end
        ...
        if(alphaOptimizerOn)
            ...
            for currentExaminedSegment = 1 : numberOfExaminedSegments
                ...
                for(k=1:length(alphaOptimizer))
                    ...
                    optimizedSignal=A*exp(alphaOptimizer(k)*alpha*(signalVector)).* cos(w*(signalVector)+phase);
                    
                    % create "evaluation" for each modification of alpha
                    evaluationOfAlphaOptimizer(k)=sum(abs(sig(signalVector(1):signalVector(end))-optimizedSignal(:)));
                end
                ...
                newOptimizedAlphaValues(currentExaminedSegment) = alphaOptimizer(indexAlpha)*alpha;
            end
            
            newValue = sum(newOptimizedAlphaValues(:))/length(newOptimizedAlphaValues);
            
            if(displayOn)
                fprintf('\t\tOptimized alpha in %d. cycle -> changed from %f to %f\n', index, alpha, newValue);
                fprintf('\n');
            end
            alpha = newValue;
        end
        ...
        if(amplitudeOptimizerOn)
            ...
            for currentExaminedSegment = 1 : numberOfExaminedSegments
                ...
                for(k=1:length(amplitudeOptimizer))
                    % generate signal with modified damping constant
                    % sig=A*exp(alpha*(0:n-1)).*cos(w*(0:n-1)+ph);
                    optimizedSignal=A*amplitudeOptimizer(k)*exp(alpha*(signalVector)).* cos(w*(signalVector)+(phase));
                    
                    % create "evaluation" for each modification of amplitude
                    evaluationOfAmplitudeOptimizer(k)=sum(abs(sig(signalVector(1):signalVector(end))-optimizedSignal(:)));
                end
                ...
                newOptimizedAmplitudeValues(currentExaminedSegment)=amplitudeOptimizer(indexAmplitude)*A;
            end
            
            newValue = sum(newOptimizedAmplitudeValues(:))/length(newOptimizedAmplitudeValues);
            
            if(displayOn)
                fprintf('\t\tOptimized amplitude in %d. cycle -> changed from %f to %f\n', index, A, newValue);
                fprintf('\n');
            end
            A = newValue;
        end
...
if(displayOn)
    ...
    fprintf('\tA     optimized from %.10f to  %.10f\n',parameters(1), parametersO(1));
    fprintf('\talpha optimized from %.10f to  %.10f\n',parameters(2), parametersO(2));
    fprintf('\tw     optimized from %.10f to  %.10f\n',parameters(3), parametersO(3));
    fprintf('\tphase optimized from %.10f to  %.10f\n',parameters(4), parametersO(4));
    fprintf('\n');
end
% =========================================================================
...
subplot(2,1,1)
plot(0:n-1,sig,'r.-',0:n-1,sigRO,'b.-');
title('Original signal vs reconstructed and optimized signal');
